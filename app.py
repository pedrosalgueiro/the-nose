...
DEFAULT_MODEL_PATH = "models/electronic_nose_fast.joblib"
DEFAULT_LOGO_PATH = "assets/electronic_nose_logo_app_light.png"
...
if st.session_state.running and sample_elapsed_s >= PREVIEW_START_S and sample_elapsed_s < LOCK_S:
    # During the 60–90 s evaluation phase, use only the data actually collected
    # so far. Do not pad the remaining seconds with the latest value, otherwise
    # the preview can appear frozen from the first prediction.
...
if sample_elapsed_s >= LOCK_S and st.session_state.final_prediction is None and session_rows:
    # At 90 s there is enough real data for the fast window, so the final result
    # is computed without synthetic future padding.
...
header_left, header_right = st.columns([3.6, 1.4], vertical_alignment="center")

with header_left:
    # Electronic Nose logo and title stay together on the left.
    logo_col, title_col = st.columns([0.14, 0.86], vertical_alignment="center")
    with logo_col:
...
with header_right:
    # VISTALab logo aligned to the right edge of the header.
    vistalab_logo_path = resolve_path(VISTALAB_LOGO_PATH)
    if SHOW_LOGOS and vistalab_logo_path.exists():
        import base64

        vistalab_b64 = base64.b64encode(vistalab_logo_path.read_bytes()).decode("ascii")
        st.markdown(
            f"""
            <div style="
                width:100%;
            ...
            ">
                <img src="data:image/png;base64,{vistalab_b64}" style="
                    width:190px;
                    max-width:100%;
                    height:auto;
                    object-fit:contain;
        ...
        st.markdown(
            "<div style='text-align:right;'>VISTALab · Centro ALGORITMI</div>",
            unsafe_allow_html=True,
        )
    ...
    if model_bundle is None:
        ...
        if sample_prediction.get("error"):
            ...
            if not final_prediction:
                st.caption(f"Atualiza com os dados reais até congelar aos {LOCK_S} s.")
