...
if st.session_state.running and sample_elapsed_s < LOCK_S and len(session_rows) >= 2:
    # Show sample probabilities from the beginning of the sampling session.
    # The prediction uses only real data collected so far and keeps updating
    # until the final lock at 90 s.
...
with controls_c:
    st.caption(
        ...
        "90–180 s recuperação. As probabilidades aparecem desde o início e congelam aos 90 s."
    )
...
with left:
    ...
    if model_bundle is None:
        ...
        if st.session_state.running:
            ...
            st.caption("As probabilidades aparecem assim que existirem leituras suficientes para a primeira janela.")
        else:
            st.metric("Resultado", "—")
        render_probability_list(empty_probs(model_bundle))
    else:
        ...
        if sample_prediction.get("error"):
            ...
            if not final_prediction:
                st.caption(f"Probabilidades em direto; atualizam com os dados reais até congelar aos {LOCK_S} s.")
...
with st.expander("Detalhes técnicos"):
    ...
    with col2:
        st.write("**Sessão**")
        st.write("Pré-visualização: `desde o início da amostragem`")
