...
# This never freezes. It is recalculated on every refresh using the latest
# rolling 180-s window from monitor_records.
if len(monitor_records) >= FULL_WINDOW_SIZE:
...
if continuous_prediction is None:
    missing = max(0, FULL_WINDOW_SIZE - len(monitor_records))
    st.info(
        f"A aguardar dados suficientes para classificação contínua. "
        f"Faltam aproximadamente {missing} leituras para uma janela de {FULL_WINDOW_SIZE} s."
    )
else:
    ...
    with c1:
        ...
        st.caption(
            "Esta classificação nunca congela: usa sempre a janela deslizante "
            "mais recente de ~180 s."
        )
