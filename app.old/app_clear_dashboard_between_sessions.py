...
DEFAULT_FULL_MODEL_PATH = "models/electronic_nose_indoor_outdoor_lemon.joblib"
...
def reset_measurement() -> None:
    ...
    # Force one clean rerun with an empty dashboard before drawing the new
    # session charts. This prevents old Streamlit chart elements from staying
    # visible under the new ones when starting sample 2, 3, ...
    st.session_state.force_clear_dashboard_once = True
...
# If a new session was just started, render one intentionally empty dynamic area
# and immediately rerun. This clears any old chart DOM left by the previous
# session before the new session charts are drawn.
if st.session_state.get("force_clear_dashboard_once", False):
    st.session_state.force_clear_dashboard_once = False
    st.info("A preparar nova medição…")
    time.sleep(0.15)
    st.rerun()
    st.stop()
...
with details_tab:
    ...
    with details_right:
        ...
        st.caption(f"Sessão atual: {st.session_state.get('session_id', 0)}")
