...
def render_probability_bars(probs: dict[str, float]) -> None:
    """
    Render all probability bars as a single HTML block.

    Using many st.progress elements can occasionally leave duplicated visual
    elements after fast reruns/session resets. A single HTML block is lighter
    and more stable for the dashboard.
    """
    if not probs:
    ...
    for label, prob in sorted(probs.items(), key=lambda item: item[1], reverse=True):
        ...
        rows.append(
            f"""
            <div class="prob-row">
                <div class="prob-label">{safe_label}</div>
                <div class="prob-track">
                    <div class="prob-fill" style="width:{pct:.2f}%"></div>
                    <div class="prob-text">{pct:.1f}%</div>
                </div>
            </div>
            """
        )
