...
def predict_from_records(records: list[dict[str, Any]], bundle: dict[str, Any] | None, mode: str) -> dict[str, Any] | None:
    if bundle is None:
        return None
...
if should_read:
    ...
    # A sessão usada para previsão congela aos 180 s.
...
full_prediction = predict_from_records(session_records, full_bundle, mode="full")
...
if st.session_state.running and elapsed >= SESSION_END_S:
    final_pred = predict_from_records(session_records, full_bundle, mode="full")
    if final_pred is not None:
        st.session_state.final_prediction_locked = final_pred
...
display_full_prediction = (
    st.session_state.final_prediction_locked
    if st.session_state.final_prediction_locked is not None
    else full_prediction
)
...
with m4:
    st.metric(
        "Previsão final",
        "Congelada" if st.session_state.final_prediction_locked is not None else (
            "Pronta" if display_full_prediction is not None else "A aguardar"
        ),
    )
...
with pred_col2:
    ...
    if full_bundle is None:
        ...
        if st.session_state.final_prediction_locked is not None:
            st.caption("Resultado final congelado aos 180 s.")
