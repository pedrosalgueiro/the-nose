...
with control_c:
    st.info(
        ...
        "35–90 s exposição/classificação, 90–180 s recuperação. A previsão congela aos 90 s."
    )
...
with metric_cols[2]:
    st.metric("Classificação", "Pronta" if (display_fast_prediction is not None or final_preview_prediction is not None) else "A aguardar")
...
if display_full_prediction is not None and st.session_state.running:
    st.success("Classificação congelada; a monitorização da recuperação continua até ao fim da sessão.")
    ...
    with dash_left:
        ...
        if fast_bundle is None:
            ...
            if display_full_prediction is not None:
                ...
                metric_name = "Resultado em tempo real"
                caption = "Pode mudar até aos 90 s."
            else:
                metric_name = "Resultado preliminar"
                caption = "Primeira previsão disponível."
