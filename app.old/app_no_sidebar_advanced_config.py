...
# Demo defaults. Configuration is hidden by default to maximize screen space.
...
with st.expander("⚙️ Configuração avançada", expanded=False):
    ...
    with cfg_col1:
        read_interval_ms = st.number_input(
            "Intervalo de leitura (ms)",
            min_value=250,
            max_value=10000,
            value=read_interval_ms,
            step=250,
            help="Tempo entre atualizações da app. 1000 ms = 1 segundo.",
        )

        broker = st.text_input("MQTT broker", value=broker)
        port = st.number_input("MQTT port", min_value=1, max_value=65535, value=port)

    with cfg_col2:
        username = st.text_input("MQTT username", value=username)
        password = st.text_input("MQTT password", value=password, type="password")

        full_model_path = st.text_input(
            "Modelo completo",
            value=full_model_path,
        )

        fast_model_path = st.text_input(
            "Modelo rápido",
            value=fast_model_path,
        )

    with cfg_col3:
        st.caption("Monitorização da recuperação")
        st.write(
            "Não usa recovery sessions/classes. Usa comparação com a baseline inicial."
        )
        st.write(
            "As leituras são reamostradas para 1 Hz antes das previsões."
        )
...
with metric_cols[0]:
    st.metric("Tempo", f"{min(elapsed, SESSION_END_S):.0f} s")
...
with metric_cols[4]:
    st.metric("Nova amostra?", "Sim" if ready else "Não")

st.progress(min(elapsed / SESSION_END_S, 1.0))
st.caption(status_text(elapsed, st.session_state.session_finished))
...
with dashboard_tab:
    dash_left, dash_mid, dash_right = st.columns(3)
    ...
    with dash_right:
        ...
        if full_bundle is None and fast_bundle is None:
            ...
            # Use fast model labels initially; fall back to full model labels.
...
with details_tab:
    ...
    with details_right:
        st.markdown("### Estado da câmara")

        if st.session_state.baseline_locked:
            st.caption(f"Baseline: primeiros {READY_BASELINE_END_S:.0f} s.")
        else:
            st.caption("Baseline ainda não bloqueada.")

        if ready:
            st.success("Pronta / clean air")
            st.metric("Pode testar?", "Sim")
        else:
            st.warning(ready_info.get("state", "A recuperar"))
            st.metric("Pode testar?", "Não")
        ...
        if checks:
            checks_df = pd.DataFrame(
                [{"critério": key, "ok": "✅" if bool(value) else "❌"} for key, value in checks.items()]
            )
            st.dataframe(
                checks_df,
        ...
        if details:
            ...
            st.write("Sem detalhes disponíveis.")
