...
with details_tab:
    ...
    with details_right:
        ...
        st.markdown("### Diagnóstico por baseline")
        ...
        st.caption(
            "Este diagnóstico é mais rígido e serve apenas como apoio; "
            "o estado principal usa a classificação contínua."
        )

        checks = baseline_ready_info.get("checks", {})
        if checks:
        ...
        details = baseline_ready_info.get("details", {})
        if details:
            with st.expander("Desvios face à baseline", expanded=False):
