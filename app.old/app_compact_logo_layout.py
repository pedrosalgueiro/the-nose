...
with st.sidebar.expander("Monitorização da recuperação"):
    ...
    st.write("As leituras são reamostradas para 1 Hz antes das previsões, porque a app pode não conseguir exatamente 1 leitura por segundo.")
...
with control_b:
    if st.button("⏹️ Parar leitura", use_container_width=True):
        stop_measurement()
...
# elapsed = current real session time, based on monitor_records/time.
# session_elapsed = last timestamp actually stored in the frozen session_records.
elapsed = 0.0
if monitor_records:
...
# Continuous rolling classification.
# This never freezes.
# It starts as soon as the fast model can produce a result (~60 s),
# then switches to the full rolling 180-s model when enough data exists.
if monitor_records and float(monitor_records[-1].get("elapsed_s", 0.0)) >= FULL_WINDOW_SIZE:
...
with m2:
    effective_points = len(resample_records_to_1hz(session_records, duration_s=FULL_WINDOW_SIZE, pad_to_duration=False))
    st.metric("Leituras da sessão", f"{len(session_records)} reais / {effective_points} úteis")
...
with pred_col1:
    st.markdown("### ⚡ Previsão rápida (~60 s)")
    if fast_bundle is None:
        ...
        st.bar_chart(pd.Series(display_fast_prediction["probs"]).sort_values(ascending=False), height=190)

with pred_col2:
    st.markdown("### 🎯 Modelo final / completo")
    ...
    if full_bundle is None:
        ...
        st.bar_chart(pd.Series(display_full_prediction["probs"]).sort_values(ascending=False), height=190)
    elif final_preview_prediction is not None:
        st.metric(
            "Pré-visualização do modelo final",
            final_preview_prediction["label"],
            f"{final_preview_prediction['confidence'] * 100:.1f}% confiança",
        )
        st.caption(
            "Pré-visualização pedagógica: pode mudar até aos 180 s. "
            "A previsão oficial só é congelada no fim da sessão."
        )
        st.bar_chart(pd.Series(final_preview_prediction["probs"]).sort_values(ascending=False), height=190)

        preview_history = st.session_state.final_preview_history
        if len(preview_history) >= 2:
            preview_df = pd.DataFrame(preview_history)
            prob_cols = [
                col for col in preview_df.columns
                if col not in {"elapsed_s", "label", "confidence", "mode"}
            ]
            if prob_cols:
                st.write("Evolução da pré-visualização até aos 180 s")
                st.line_chart(preview_df.set_index("elapsed_s")[prob_cols], height=180)
    else:
        st.info("A pré-visualização do modelo final aparece após ~60 s; o resultado oficial fica aos 180 s.")


# -------------------------------------------------------------------
# Continuous rolling classification UI
# -------------------------------------------------------------------

st.markdown("## 🔄 Classificação contínua")
...
if continuous_prediction is None:
    ...
    st.info(
        f"A aguardar dados suficientes para classificação contínua. "
        f"Faltam aproximadamente {missing} s para a primeira previsão contínua."
    )
else:
    ...
    with c1:
        ...
        if continuous_mode == "rolling_full_180s":
            st.caption("Classificação contínua: janela deslizante completa de ~180 s.")
        else:
            st.caption("Classificação contínua inicial: janela rápida de ~60 s.")

    with c2:
        st.bar_chart(pd.Series(continuous_prediction["probs"]).sort_values(ascending=False), height=190)
    ...
    if len(history) >= 2:
        ...
        if prob_cols:
            st.write("Evolução contínua das probabilidades")
            st.line_chart(history_df.set_index("elapsed_s")[prob_cols], height=180)
...
st.markdown("## ✅ Estado da câmara para nova amostra")

if st.session_state.baseline_locked:
    st.caption(f"Baseline de referência bloqueada com os primeiros {READY_BASELINE_END_S:.0f} s da sessão.")
else:
    st.caption("Baseline de referência ainda não bloqueada. Mantém ar limpo no início da sessão.")
...
with st.expander("Leituras atuais dos sensores", expanded=False):
    if monitor_records:
        latest_record = monitor_records[-1]
        sensor_df = pd.DataFrame(
            [{"sensor": col, "value": latest_record.get(col)} for col in SENSOR_COLUMNS]
        )
        st.dataframe(sensor_df, use_container_width=True, hide_index=True, height=260)
    else:
...
if len(session_records) >= 2:
    ...
    if selected_cols:
        st.line_chart(df_session.set_index("elapsed_s")[selected_cols], height=230)
else:
...
with st.expander("Histórico contínuo / recuperação"):
    if len(monitor_records) >= 2:
        ...
        if available_recovery_cols:
            st.line_chart(df_monitor.set_index("elapsed_s")[available_recovery_cols], height=220)
    else:
