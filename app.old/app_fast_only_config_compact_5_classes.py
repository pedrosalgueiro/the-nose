...
def load_app_config(config_path: str | Path) -> tuple[dict[str, Any], Path | None, str | None]:
    ...
    default_config: dict[str, Any] = {
        ...
        "ui": {
            "probability_max_classes": 3,
            "probability_min_percent": 1.0,
            "show_other_probability": True,
            "show_monitor_probabilities_during_session": False,
...
def render_probability_bars(probs: dict[str, float], *, height_hint: str = "compact") -> None:
    ...
    With 5 classes, showing every class in every panel makes the dashboard too
    tall. This view shows only the most relevant classes and optionally merges
    the rest into "outras".
    """
    if not probs:
        st.info("Sem probabilidades para mostrar.")
        return

    max_classes = max(1, int(globals().get("PROBABILITY_MAX_CLASSES", 3)))
    min_prob = max(0.0, float(globals().get("PROBABILITY_MIN_PERCENT", 1.0))) / 100.0
    show_other = bool(globals().get("SHOW_OTHER_PROBABILITY", True))

    sorted_items = [
        (str(label), max(0.0, min(1.0, float(prob))))
    ...
    for idx, item in enumerate(sorted_items):
        label, prob = item
        if idx < max_classes and (idx == 0 or prob >= min_prob):
            visible.append(item)
        else:
            hidden.append(item)

    if show_other and hidden:
        other_prob = sum(prob for _, prob in hidden)
        if other_prob > 0:
            visible.append(("outras", min(1.0, other_prob)))

    for label, prob in visible:
        pct = prob * 100.0

        left, right = st.columns([1.15, 4.8])

        with left:
            st.markdown(
                f"<div style='font-size:0.82rem;font-weight:650;padding-top:0.05rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{label}</div>",
                unsafe_allow_html=True,
            )

        with right:
            # Keep this compact: Streamlit's default progress row is a bit tall,
            # so limit the number of rows rather than trying to show all classes.
            st.progress(
                max(0.0, min(1.0, prob)),
                text=f"{pct:.1f}%",
            )

    if hidden and not show_other:
        st.caption(f"+ {len(hidden)} classes ocultas para manter o dashboard compacto.")
...
PROBABILITY_MAX_CLASSES = int(ui_settings.get("probability_max_classes", 3))
PROBABILITY_MIN_PERCENT = float(ui_settings.get("probability_min_percent", 1.0))
SHOW_OTHER_PROBABILITY = bool(ui_settings.get("show_other_probability", True))
SHOW_MONITOR_PROBS_DURING_SESSION = bool(
    ui_settings.get("show_monitor_probabilities_during_session", False)
)
...
with dashboard_tab:
    dash_left, dash_mid, dash_messages = st.columns([1.45, 1.0, 0.95])

    with dash_left:
        st.markdown("### 🧠 Classificação da amostra")

        # Only one model is used in this version: the fast model.
        # During the session, show the best available prediction from that model.
        # At 180 s, show the frozen result.
        sample_prediction = (
        ...
        if fast_bundle is None:
            ...
            st.caption("O modelo rápido começa a prever após cerca de 60 s.")
            render_probability_bars(empty_probs_for_bundle(fast_bundle))
        else:
            if display_full_prediction is not None:
                metric_name = "Resultado congelado"
                caption = "Resultado rápido congelado aos 180 s."
                ...
                caption = "Primeira previsão do modelo rápido."
    ...
    with dash_mid:
        panel_title = "🧭 Estado da sessão" if st.session_state.running else "🔄 Monitorização contínua"
        st.markdown(f"### {panel_title}")

        if fast_bundle is None:
            st.warning("Modelo rápido não encontrado.")
            render_probability_bars(empty_probs_for_bundle(None))
        elif continuous_prediction is None:
            if st.session_state.running:
                missing = max(0, int(60 - elapsed))
                render_waiting_metric("Sessão atual", f"~{missing} s")
                st.caption("Durante a sessão, este painel acompanha a mesma amostra.")
            else:
                ...
                st.caption("Fora da sessão, acompanha a recuperação / ar limpo.")
            render_probability_bars(empty_probs_for_bundle(fast_bundle))
        else:
            ...
            if st.session_state.running:
                st.caption("Alinhado com a classificação da amostra.")
                if SHOW_MONITOR_PROBS_DURING_SESSION:
                    render_probability_bars(continuous_prediction["probs"])
                else:
                    st.caption("Probabilidades omitidas aqui para evitar scroll; vê o painel da amostra.")
            else:
                st.caption("Janela rápida deslizante.")
                render_probability_bars(continuous_prediction["probs"])
...
with details_tab:
    ...
    with st.expander("⚙️ Configuração avançada", expanded=False):
        ...
        with cfg_col2:
            ...
            st.write(f"Classes visíveis nas probabilidades: `{PROBABILITY_MAX_CLASSES}`")
            st.write(f"Probabilidades no painel de monitorização durante sessão: `{SHOW_MONITOR_PROBS_DURING_SESSION}`")
