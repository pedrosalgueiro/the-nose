...
with col_c:
    st.info(
        "Protocolo: 0–20 s ar limpo estável, 20–35 s aproximar amostra, "
...
with pred_col2:
    st.subheader("Modelo final / completo")
    ...
    if full_bundle is None:
        ...
        st.bar_chart(pd.Series(final_preview_prediction["probs"]).sort_values(ascending=False))
        ...
        if len(preview_history) >= 2:
            ...
            if prob_cols:
                ...
                st.line_chart(preview_df.set_index("elapsed_s")[prob_cols])
    else:
...
st.header("Classificação contínua")
